...
<div class='widget-box'>
    <div class='widget-title'>
        <div class="buttons">
        <?php if(checkPermission(array($projectID,'events',$eventID,'hospitality','types','create'))): ?>
	        <button class="btn btn-success" onClick="newHospTypeModal()"><i class="icon-plus"></i> Add New Hospitality Type</button>
        <?php endif; ?>
        </div>
		<ul class="nav nav-tabs">
			<li class="<?=(!isset($site['page'][EVENT_INDEX+1]) || !in_array($site['page'][EVENT_INDEX+1],array('admin','order')))?"active":""?>">
            	<a href="#hospitality" data-toggle="tab">Hospitality</a>
            </li>
            <?php if(checkPermission(array($projectID,'events',$eventID,'hospitality','admin'))): ?>
			<li class="<?=(isset($site['page'][EVENT_INDEX+1]) && $site['page'][EVENT_INDEX+1] == "admin")?"active":""?>">
            	<a href="#hospitality-admin" data-toggle="tab">Admin</a>
            </li>
<?php /* ?>
			<li><a href="#hospitality-packs" data-toggle="tab">Addons</a></li>
<?php */ ?>
            <?php endif; ?>
            <li class="<?=(isset($site['page'][EVENT_INDEX+1]) && $site['page'][EVENT_INDEX+1] == "oder")?"active":""?>">
            	<a href='#hospitality-purchase' data-toggle='tab'>Request Hospitality</a>
            </li>
		</ul>
    </div>
    <div class='widget-content tab-content'>
        <div id="hospitality" class="tab-pane <?=(!isset($site['page'][EVENT_INDEX+1]) || !in_array($site['page'][EVENT_INDEX+1],array('admin','order')))?"active":""?>">
			<?php require_once(dirname(__FILE__)."/hospitality-hospitality.php") ?>
        </div>
        <div id="hospitality-admin" class="tab-pane <?=(isset($site['page'][EVENT_INDEX+1]) && $site['page'][EVENT_INDEX+1] == "admin")?"active":""?>">
			<?php require_once(dirname(__FILE__)."/hospitality-admin.php") ?>
        </div>
<?php /* ?>
        <div id="hospitality-packs" class="tab-pane"><?php require_once(dirname(__FILE__)."/hospitality-packs.php") ?></div>
<?php */ ?>
		<div id="hospitality-purchase" class="tab-pane <?=(isset($site['page'][EVENT_INDEX+1]) && $site['page'][EVENT_INDEX+1] == "order")?"active":""?>">
			<?php require_once(dirname(__FILE__)."/hospitality-purchase.php") ?>
        </div>
    </div>
</div>
<div class="modal hide" id="addHospTypeModal">
    ...
    <form accept-charset="UTF-8" action="<?=SITE_ROOT?>event.hospitality/<?=$projectID?>/<?=$eventID?>/" method="post" class="form-horizontal">
    	<input type="hidden" name="hospTypeID">
    	<input type="hidden" name="action">
        <input type="hidden" name="eventID" value="<?= $eventID ?>">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            <h3><span class="actionName"></span> Hospitality Type</h3>
        </div>
        <div class="modal-body">
            ...
            <div class="control-group">
                <label class="control-label">Hospitality Name:</label>
                <div class="controls">
                	<input type="text" name="name" placeholder="Hospitality Name">
        ...
        <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-primary"><span class="actionName"></span> Hospitality Type</button>
        </div>
    </form>
</div>
<script>
